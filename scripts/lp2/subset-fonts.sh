#!/usr/bin/env bash
# lp-2 用のサブセットフォントを書き出す(docs/spec/07_lp2-renewal.md §15.7)。
#
#   ./scripts/lp2/subset-fonts.sh            # 収録文字は charset ファイルから
#   LP2_URL=... ./scripts/lp2/subset-fonts.sh --collect   # 実ページから採取し直す
#
# next/font 経由で Google Fonts から配信すると、日本語フォントは unicode-range で
# 100 以上のチャンクに分割され、lp-2 の 1 ページで 633KB を取得していた。
# 実際に使う文字だけの woff2 にすると 215KB になる。
#
# lp-1 は従来どおり next/font を使う。ここで扱うのは lp-2 専用の 2 書体だけ。
set -euo pipefail

cd "$(dirname "$0")/../.."
PY="${LP2_PYTHON:-./.venv-lp2/bin/python}"
[ -x "$PY" ] || PY="python3"
OUT="public/fonts/lp-2"
SRC="${LP2_FONT_SRC:-tmp/font-src}"
CHARSET_DIR="scripts/lp2/charsets"
