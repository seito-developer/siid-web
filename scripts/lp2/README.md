# LP リニューアル(`/siid/lp-2`)用スクリプト

Photoshop 入稿データから実装に必要な情報とアセットを取り出し、
実装結果をデザインと突き合わせるためのツール一式。

仕様: [`docs/spec/07_lp2-renewal.md`](../../docs/spec/07_lp2-renewal.md)
...
```bash
python3 -m venv .venv-lp2
./.venv-lp2/bin/pip install -r scripts/lp2/requirements.txt
...
```bash
V=./.venv-lp2/bin/python
PSD=~/Downloads/260902_seitosama

# レイヤーツリーを見る(調査用)
$V scripts/lp2/psd_tool.py tree $PSD/pc/pc1.psd --filter 'fv/*' --max-depth 2

# 基準画像を書き出す(デザイン差分判定に使う)
$V scripts/lp2/psd_tool.py render $PSD/pc --out-dir tmp/psd-ref/pc
$V scripts/lp2/psd_tool.py render $PSD/sp --out-dir tmp/psd-ref/sp

# レイヤーツリーを JSON 化する(細部を機械的に調べたいとき)
$V scripts/lp2/psd_tool.py data $PSD/pc --out-dir tmp/psd-data/pc

# アセットを一括書き出しする
$V scripts/lp2/psd_tool.py manifest scripts/lp2/assets.manifest.json
```
...
```bash
$V scripts/lp2/gen-section-docs.py $PSD --out-dir docs/spec/lp2-sections
```
