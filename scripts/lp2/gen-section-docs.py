...
ようにするためのもの(docs/spec/07_lp2-renewal.md §3)。

  python3 scripts/lp2/gen-section-docs.py ~/Downloads/260902_seitosama \\
      --out-dir docs/spec/lp2-sections

出力: docs/spec/lp2-sections/NN-<slug>.md (DOM 順に採番)
...
def build(psd_dir: str, out_dir: str) -> None:
    ...
    for order, (slug, title, pc, sp) in enumerate(SECTIONS, start=1):
        ...
        lines.append("> `scripts/lp2/gen-section-docs.py` による自動生成。手で編集しないこと。")
    ...
    readme = [
        ...
        "",
        "生成: `python3 scripts/lp2/gen-section-docs.py <PSD ディレクトリ> --out-dir docs/spec/lp2-sections`",
        "",
        "並び順は DOM 順(`docs/spec/07_lp2-renewal.md` §3.1 の決定に従い FAQ → FREE GIFTS)。",
        "",
        "| # | セクション | コンポーネント | 補足 |",
        "|---|-----------|--------------|------|",
        *index,
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", default="docs/spec/lp2-sections")
