...
// LP_CAREER_TEST_ORIGIN=http://localhost:3001 node scripts/lp-career/check-booking-layout.cjs
const origin = process.env.LP_CAREER_TEST_ORIGIN || "http://localhost:3102";
...
(async () => {
  ...
  for (const width of [1440, 839, 390, 319]) {
    ...
    await page.goto(`${origin}/siid/lp-career#counselling`);
    ...
    await frame.evaluate(
      ...
      width < 768 ? "/siid/lp-career/complete" : `${origin}/siid/lp-career/complete`,
    );
    await page.waitForURL("**/lp-career/complete");
    await page.close();
  }
  await browser.close();
})().catch((e) => {
