#!/usr/bin/env bash
# セクション背景を PSD から書き出す(docs/spec/07_lp-career-renewal.md §8.2)。
#
#   ./scripts/lp-career/export-bg.sh          # 全部
#   ./scripts/lp-career/export-bg.sh strength # 1 つだけ
#
...
[ -x "$PY" ] || PY="python3"
PSD_DIR="${LP_CAREER_PSD_DIR:-$HOME/Downloads/260902_seitosama}"
...
run_one() {
  ...
  "$PY" scripts/lp-career/psd_tool.py section-bg "$PSD_DIR/$psd.psd" \
    --hide-kinds type "${args[@]}" \
    --box "$box" --match-reference "$ref" \
    --out "public/images/lp-career/$out.webp" || return 1
  if [ "$name" = "pc-cta-button" ]; then node scripts/lp-career/clean-cta.mjs; fi
}
