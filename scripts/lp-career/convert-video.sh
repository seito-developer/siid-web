#!/usr/bin/env bash
# FV 背景動画を Web 配信用に変換する(docs/spec/07_lp-career-renewal.md §9)。
#
#   ./scripts/lp-career/convert-video.sh <入力ディレクトリ>
#
# 入力: <入力ディレクトリ>/fv-pc.mov, fv-sp.mov
# 出力: public/videos/lp-career/{fv-pc,fv-sp}.mp4 と同名の -poster.webp
#
# 設計判断:
...
OUT_DIR="$(cd "$(dirname "$0")/../.." && pwd)/public/videos/lp-career"
mkdir -p "$OUT_DIR"
