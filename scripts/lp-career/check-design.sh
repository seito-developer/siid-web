...
#
#   ./scripts/lp-career/check-design.sh pc
#
# 事前に `npm run dev` を起動し、基準画像を書き出しておくこと:
#   python3 scripts/lp-career/psd_tool.py render <PSD ディレクトリ> --out-dir tmp/psd-ref/<pc|sp>
set -o pipefail
...
# 別カットになることが承認済みのため(docs/spec/lp-career-sections/02-fv.notes.md)。
...
for row in "${ROWS[@]}"; do
  ...
  if [ -n "$ignore" ]; then
    out=$("$PY" scripts/lp-career/compare.py --reference "$ref" --actual "$shot" \
