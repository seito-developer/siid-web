...
//
//   node scripts/lp-career/shoot-fullpage.mjs 1440
//   node scripts/lp-career/shoot-fullpage.mjs 375
//
...
await p.goto(process.env.LP_CAREER_URL ?? 'http://localhost:3001/siid/lp-career', { waitUntil: 'domcontentloaded', timeout: 60000 });
...
const info = await p.evaluate(() => ({
  sections: Object.fromEntries([...document.querySelectorAll('section[id]')].map(el => {
    const r = el.getBoundingClientRect();
    return [el.id, { top: Math.round(r.top + scrollY), height: Math.round(r.height) }];
  })),
...
const out = process.env.SPDIR ?? 'tmp/shots';
await fs.mkdir(out, { recursive: true });
await fs.writeFile(`${out}/sections-${w}.json`, JSON.stringify(info, null, 2));
