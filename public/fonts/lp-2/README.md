...
`/siid/lp-2` で実際に使う文字だけを含む woff2。next/font 経由で Google Fonts から
...
そのため lp-2 のトークン(`--lp2-font-body` など)をサブセット側に向けるだけで、
next/font 側の登録はそのままでも取得は起きない。**lp-1 の設定は一切変えていない。**

- 生成: `./scripts/lp2/subset-fonts.sh`
- ライセンス: SIL Open Font License 1.1(同梱の OFL-*.txt)。
  いずれも Reserved Font Name の指定は無いため、サブセット化と再配布が可能。
- **lp-1 は従来どおり next/font を使う。** ここのフォントは lp-2 専用。

収録文字は「描画済みの DOM から採取した文字」+「lp-2 のソース中の文字列リテラルと
