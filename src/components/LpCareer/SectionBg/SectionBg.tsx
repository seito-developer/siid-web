...
// (docs/spec/07_lp-career-renewal.md §8.2)。SP のカンプは @2x なので
// 実寸の半分で表示される。
//
...
type Props = {
  /** public/images/lp-career/<name>-bg.webp と public/images/lp-career/sp/<name>.webp を使う */
  name: string;
  pcWidth: number;
  pcHeight: number;
  spWidth: number;
...
export default function SectionBg({
  ...
  const common = { alt: '', sizes: '100vw', quality: LP_CAREER_IMAGE_QUALITY };

  const {
    props: { srcSet: spSrcSet },
  } = getImageProps({
    ...common,
    src: lpCareerAsset(`/images/lp-career/sp/${name}.webp`),
    width: spWidth,
    height: spHeight,
  });
  ...
  const {
    ...
    ...common,
    src: lpCareerAsset(`/images/lp-career/${name}-bg.webp`),
    width: pcWidth,
    height: pcHeight,
  });
