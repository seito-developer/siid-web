...
import { LP_CAREER_PLANS } from '@/constants/lpCareerPlans';
...
// PRICING(docs/spec/lp-career-sections/11-plan.md)。
    ...
    <section className={styles.Plan} id="plan">
      ...
      <div className={styles.Plan__Inner}>
        ...
        <Image
          ...
          src={lpCareerAsset('/images/lp-career/plan-instructor.webp')}
          alt=""
          width={179}
          height={358}
        ...
        <ul className={styles.Plan__Tabs} role="tablist" aria-label="料金プラン">
          {LP_CAREER_PLANS.map((plan, i) => (
            <li key={plan.id} role="presentation">
              <button
                type="button"
                role="tab"
                id={`lp-career-plan-tab-${plan.id}`}
                aria-selected={i === activeIndex}
                aria-controls={`lp-career-plan-panel-${plan.id}`}
        ...
        <div className={styles.Plan__Cards}>
          {LP_CAREER_PLANS.map((plan, i) => (
            <div
              key={plan.id}
              id={`lp-career-plan-panel-${plan.id}`}
              role="tabpanel"
              aria-labelledby={`lp-career-plan-tab-${plan.id}`}
            ...
            >
              ...
              <Image
                ...
                src={lpCareerAsset(`/images/lp-career/${plan.chrome}.webp`)}
                alt=""
