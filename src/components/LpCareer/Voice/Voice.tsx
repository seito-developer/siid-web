...
import { LP_CAREER_VOICES } from '@/constants/lpCareerVoices';
...
// VOICE(docs/spec/lp-career-sections/13-voice.md)。
// 「続きを読む」はモーダルではなくカード内で伸びるアコーディオン。

function Card({ voice, index }: { voice: (typeof LP_CAREER_VOICES)[number]; index: number }) {
  const [isOpen, setIsOpen] = useState(false);
  const bodyId = `lp-career-voice-body-${index}`;
