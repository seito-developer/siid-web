...
import { LP_CAREER_RESULTS } from '@/constants/lpCareerResults';
...
// RESULTS(docs/spec/lp-career-sections/04-result.md)。
...
function isNear(index: number, active: number) {
  const last = LP_CAREER_RESULTS.length - 1;
  const distance = Math.min(
    Math.abs(index - active),
    Math.abs(index - active + LP_CAREER_RESULTS.length),
    Math.abs(index - active - LP_CAREER_RESULTS.length),
  );
...
export default function Result() {
  ...
  const active = LP_CAREER_RESULTS[activeIndex] ?? LP_CAREER_RESULTS[0];
          ...
          {LP_CAREER_RESULTS.map((item, i) => (
            <SwiperSlide key={item.image} className={styles.Result__Slide}>
              ...
              {isNear(i, activeIndex) ? (
                <Image
                  src={lpCareerAsset(`/images/lp-career/results/${item.image}.webp`)}
