...
import { LP_CAREER_SKILLS } from '@/constants/lpCareerSkills';
...
// SKILLS(docs/spec/lp-career-sections/09-skill.md と 09-skill.notes.md)。
//
...
function Card({
  ...
  card: (typeof LP_CAREER_SKILLS)[number];
  index: number;
  isPc: boolean;
}) {
  // カンプの SP は 1 枚目だけ開いた状態
  const [isOpen, setIsOpen] = useState(index === 0);
  const panelId = `lp-career-skill-panel-${index}`;
      ...
      <div className={styles.Skill__Panel} id={panelId}>
        ...
        <ul className={styles.Skill__Tags}>
          {card.tags.map((tag) => (
            <li key={tag.label}>
              <Image
                src={lpCareerAsset(`/images/lp-career/skills/${tag.icon}.webp`)}
                alt=""
                width={22}
                height={22}
