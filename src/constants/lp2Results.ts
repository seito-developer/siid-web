...
//
// そのためカンプから読み取れた 5 件は実データを使い、残りは 2 行目を空にしている。
// 事実と異なる転職実績を捏造しないため、仮の箇所に具体的な成果は書かない。
// 公開前に実際のキャプションへ差し替えること(isPlaceholder が目印)。
export type Lp2Result = {
  image: string;
  /** 1 行目 */
  from: string;
  /** 2 行目(強調色)。未確定のスライドは空文字にして仮テキストを出さない */
  to: string;
...
export const LP2_RESULTS: Lp2Result[] = [
  ...
  { image: 'carousel-06', from: '受講生インタビュー', to: '', isPlaceholder: true },
  { image: 'carousel-07', from: '受講生インタビュー', to: '', isPlaceholder: true },
  { image: 'carousel-08', from: '受講生インタビュー', to: '', isPlaceholder: true },
];
