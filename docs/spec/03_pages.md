...
- **導線(Issue #51 で確定)**: コース一覧ページ(`/courses`)の LINE バナー(`components/Courses/LineBanner`)から `/line` へリンクする。同バナーは友だち追加 URL 直リンクではなく `/line` を経由させる(CTA 文言が「詳細はこちら」であり、`/line` 内に友だち追加 CTA があるため)。グローバルナビのメニューには `/line` を追加していない。フッターのメインメニュー(PC では 3 列目、`footerExtraMenuItems`)には Issue #131(2026-09-17)で追加した
...
- **PC 下層ナビの CONTACT ボタン【対応済み 2026-09 / Issue #148】**: TOP は `Header` の CONTACT ボタンで `/counseling` へ誘導できるが、PC の下層ページは `NavigationPcLower` にメニュー 4 項目しかなく、フッターまでスクロールしないと導線が無かった。`NavigationPcLower` の右上(青いヘッダーパネル上)に「</ CONTACT > 個別説明会へ申し込む」の 1 行ボタンを固定表示し、スクロール中も残す。配色は `ContactButton` に揃えた。グローバルメニュー(`menuItems.ts`)には追加していない(SP のハンバーガーメニューには既に `ContactButton` があり、重複するため)
  - 404 の `NotFoundHero` は上余白が 50px でボタンと重なったため、PC の上余白を他の下層ページの `Headline` と同じ 95px に揃えた
...
- **`/lp-1`【Issue #48 で削除 2026-09】** — 広告流入用の独立LP。旧LPのデザイン(navy×orange のネオブルータリズム)を忠実再現していた。lp-career への一本化により削除し、`/lp-1` は `/lp-career` へ 301。以下は実装当時の記録(root layout 分割は現在も有効。`lp1.css` は全体ルールだけを `(Lp)/lp-base.css` に切り出して削除、Poppins / Barlow と `src/components/Lp1/` も削除)
...
- **`/counseling-complete`**(と、Issue #48 で削除した `/counseling-complete-lp-1`) — 申込完了ページ(noindex)。(LowerPages) パターンで新サイトデザインに載せ替え。`/counseling-complete-lp-1` は `/lp-career/complete` へ 301(2026-09-12 決定。同ページが発火していた OpenAI Ads の CV は止まる)
...
- **`/white-paper`** — 資料請求ページ(index 可・sitemap 登録済み)。資料イメージ 2 枚 + 公式LINE誘導(`https://bit.ly/4p3SOBn`)。(LowerPages) パターン。導線はフッターのメインメニュー(`footerExtraMenuItems`、Issue #131)
