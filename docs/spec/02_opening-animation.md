...
- ローディングのロゴは退場時に**拡大しながらフェードで逃がす**(2026-07 実装時判断: FLIP モーフは効果対コストが見合わないため簡易版を採用。直後に背面ロゴが scale 1.15→1.0 で現れるため視覚的な受け渡し感は維持される)
...
| 2 | メインコピー (`HeroMainCopy(Pc)`) | **1文字ずつ**下からせり上がるスタッガー(SVG がグリフ単位の `<path>` で構成されているためこれを利用。1文字 0.7s、字間 0.035s、`back.out(1.4)`)。2026-07 実装時判断で行単位 clip-path から文字単位に変更 — より大胆な演出になるため |
