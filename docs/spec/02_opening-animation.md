...
- **`prefers-reduced-motion: reduce`**: ~~ローディングは最小限(ロゴ静止表示 + フェード)、FV は全要素即時表示~~ → **2026-09-15 変更(Issue #86)。環境を問わず演出をフル再生する**(下記)

## 完了条件(Definition of Done)

- [ ] SP(〜1279px)/ PC(1280px〜)両方で意図どおり動作
- [ ] リロード連打・遅い回線(DevTools throttling)でも破綻しない(タイムアウト遷移が機能)
- [x] `prefers-reduced-motion` の有無にかかわらず同じ演出が再生される(2026-09-15 変更。実測で両設定の挙動が一致することを確認)
...
## prefers-reduced-motion の扱い(2026-09-15 変更・Issue #86)

当初は `prefers-reduced-motion: reduce` の環境で演出を省略していたが、**環境を問わずフル再生する**方針に変更した。

### 経緯

macOS の「視差効果を減らす」を有効にしている環境では、ロゴ描画・マスクワイプ・メインコピーのスタッガーがすべて実行されず、0.4 秒でオーバーレイが消えるだけになっていた。オーナーの環境がこの設定だったため「演出が実装されていない / デグレした」ように見え、確認のたびに設定を切り替える必要が生じていた。

演出はブランドの第一印象として重視しており、環境差で見え方が変わるデメリットが上回るとオーナーが判断(2026-09-15)。

### 留意点

この設定は、動きの多い画面で乗り物酔いのような症状が出る利用者が使うもの。無視することで、その利用者にも演出が表示される。WCAG の必須要件ではない(2.3.3 Animation from Interactions は AAA、かつ本演出はユーザー操作起因ではない)ため実装上の問題は無いが、性質を理解したうえでの意図的な選択である。

### 対象外

`src/components/NotFound/Game/`(404 ページのミニゲーム)の `reduced-motion` 分岐は**変更しない**。被弾時の点滅を 3Hz 未満に抑える光過敏(発作誘発)対策であり、体調不良の回避という点で性質が異なる。
