...
### 10.1 前提情報(2026-08 実測、2026-09-12 再確認で変化なし)
...
| ドメイン有効期限 | 2027-03-13 |
| ドメインステータス | `clientTransferProhibited`(レジストラ移管ロック)・`clientDeleteProhibited`(削除ロック)。**ネームサーバ変更にはどちらも解除不要** |
...
- CAA レコードは未設定。
- 2026-09-12 に外部から再取得し、上表の全レコード(DKIM は 408 文字の全文)が一致することを確認済み。
...
移管前の実測ベースライン(2026-08 実測、2026-09-12 再測定で同一):
