...
| 顧問プラン（企業・個人事業主向け） | **660,000 円〜**（ミニマム）/ 990,000 円〜（スタンダード）/ 1,650,000 円〜（チーム） | 対象外 | 27,500 円〜 | 期間 6 ヶ月〜。個別コンサルは月 1〜3 回。旧「Career +VIP Edition（898,000 円）」は廃止し、このプランに置き換えた |
| Basic | 128,000 円 | 対象外 | 5,333 円 | **Web サイトには載せない**（個別説明会でのダウンセル用） |

- TOP コースカード（`courseData.ts`）は給付金適用後の価格、`/courses`（`coursePlans.json`）は通常価格 + 適用後を表示。顧問プランは通常価格のみ（給付金対象外）
